...
// Resolve preset from CLAUDE_PLUGIN_OPTION_PRESET_* boolean env vars.
// Claude Code surfaces these as 5 checkboxes in /plugin UI. Most-restrictive
// wins (fail-closed): if multiple are checked, paranoid > strict > standard >
// minimal > disabled. Returns null if none set (caller falls through to
// legacy single-string env var, then to file configs, then to defaults).
function _resolvePresetFromBooleans() {
  const order = ['paranoid', 'strict', 'standard', 'minimal', 'disabled'];
...
function loadConfig() {
  ...
  const presetFromBooleans = _resolvePresetFromBooleans();
  if (presetFromBooleans) {
    pluginDefaults.preset = presetFromBooleans;
  } else if (process.env.CLAUDE_PLUGIN_OPTION_PRESET) {
    // Back-compat: pre-v2.3 single-string field (older installs may still have
    // pluginConfigs["knox@qoris"].options.preset = "<name>" in settings.json)
    pluginDefaults.preset = process.env.CLAUDE_PLUGIN_OPTION_PRESET;
  }
  ...
  // Precedence (low → high): defaults → pluginDefaults → userCfg → projectCfg → localCfg → envCfg
  const result = mergeConfigs(pluginDefaults, userCfg, projectCfg, localCfg, envCfg);
